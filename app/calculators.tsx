...
const CALCULATORS: CalcDef[] = [
  {
    ...
    fields: [
      { id: 'principal',       label: 'Initial amount',        placeholder: '10,000',  keyboardType: 'decimal-pad' },
      { id: 'annual_rate',     label: 'Annual return (%)',      placeholder: '7',       keyboardType: 'decimal-pad' },
      { id: 'years',           label: 'Duration (years)',       placeholder: '10',      keyboardType: 'number-pad'  },
      { id: 'monthly_deposit', label: 'Monthly deposit',        placeholder: '500',     keyboardType: 'decimal-pad', defaultValue: '0' },
    ],
    results: [
  ...
  },
  {
    ...
    fields: [
      { id: 'gross_monthly',    label: 'Monthly gross salary',    placeholder: '15,000', keyboardType: 'decimal-pad' },
      { id: 'income_tax_rate',  label: 'Income tax rate (%)',     placeholder: '30',     keyboardType: 'decimal-pad' },
      { id: 'ni_rate',          label: 'Social / NI rate (%)',    placeholder: '12',     keyboardType: 'decimal-pad' },
      { id: 'other_deductions', label: 'Other deductions',        placeholder: '0',      keyboardType: 'decimal-pad', defaultValue: '0' },
    ],
    results: [
  ...
  },
  {
    ...
    fields: [
      { id: 'loan_amount',  label: 'Loan amount',             placeholder: '500,000', keyboardType: 'decimal-pad' },
      { id: 'annual_rate',  label: 'Annual interest rate (%)', placeholder: '4.5',    keyboardType: 'decimal-pad' },
      { id: 'term_years',   label: 'Loan term (years)',        placeholder: '25',     keyboardType: 'number-pad'  },
    ],
    results: [
  ...
  },
  {
    ...
    fields: [
      ...
      { id: 'current_savings',      label: 'Current savings',         placeholder: '0',       keyboardType: 'decimal-pad', defaultValue: '0' },
      { id: 'monthly_contribution', label: 'Monthly contribution',    placeholder: '1,000',   keyboardType: 'decimal-pad' },
      { id: 'annual_return',        label: 'Expected annual return (%)', placeholder: '5',    keyboardType: 'decimal-pad', defaultValue: '0' },
    ],
    results: [
  ...
  },
  {
    ...
    fields: [
      { id: 'monthly_income', label: 'Monthly net income', placeholder: '12,000', keyboardType: 'decimal-pad' },
      { id: 'must_pct',       label: 'Must (%)',           placeholder: '50',     keyboardType: 'number-pad', defaultValue: '50' },
      { id: 'want_pct',       label: 'Want (%)',           placeholder: '30',     keyboardType: 'number-pad', defaultValue: '30' },
      { id: 'invest_pct',     label: 'Invest (%)',         placeholder: '20',     keyboardType: 'number-pad', defaultValue: '20' },
    ],
    results: [
  ...
  },
  {
    ...
    fields: [
      { id: 'monthly_expenses',   label: 'Monthly expenses',        placeholder: '5,000', keyboardType: 'decimal-pad' },
      { id: 'months_to_cover',    label: 'Months to cover',         placeholder: '6',     keyboardType: 'number-pad', defaultValue: '6' },
      { id: 'monthly_saving',     label: 'Monthly saving capacity', placeholder: '500',   keyboardType: 'decimal-pad' },
    ],
    results: [
...
// ── Calculation stubs ─────────────────────────────────────────────────────────
// TODO: implement each case. Return null to show blank results.
function compute(id: string, _vals: Record<string, string>): Record<string, string> | null {
  switch (id) {
    case 'compound':  return null;
    case 'salary':    return null;
    case 'loan':      return null;
    case 'savings':   return null;
    case 'budget':    return null;
    case 'emergency': return null;
    default:          return null;
  }
}
...
export default function CalculatorsScreen() {
  ...
  function openCalc(id: string) {
    ...
    def.fields.forEach((f) => { if (f.defaultValue !== undefined) defaults[f.id] = f.defaultValue; });
    setValues(defaults);
    setResult(null);
    setActiveId(id);
  }
  ...
  function handleCalculate() {
    if (!activeId) return;
    setResult(compute(activeId, values));
  }
      ...
      <AppScreen scroll>
        ...
        <View style={styles.resultCard}>
          ...
          {activeCalc.results.map((row, i) => (
            <View
              key={row.id}
              style={[styles.resultRow, i < activeCalc.results.length - 1 && styles.resultRowBorder]}
            >
